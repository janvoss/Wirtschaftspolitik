# Wirtschaftspolitik und Wirtschaftspsychologie in der „grünen Transformation“

## Einleitung

### Worin besteht die Transformation?
...
# Digraph-Objekt erstellen (von links nach rechts fließend)
dot = graphviz.Digraph(comment='Transformationsprozess', format='svg')

# Globale Layout-Einstellungen für ein elegantes, wissenschaftliches Design
dot.attr(rankdir='LR')
dot.attr('node', fontname='Arial', fontsize='11', shape='box', style='filled', color='#2c3e50')
dot.attr('edge', fontname='Arial', fontsize='10', color='#7f8c8d')

# --- AUSGANGSPUNKT ---
dot.node('START', 'Auslaufender\nEntwicklungspfad\n(Fossile Basis / Status quo)', fillcolor='#95a5a6', fontcolor='white')

# --- KERN DER TRANSFORMATION (Zentraler Prozess) ---
with dot.subgraph(name='cluster_core') as c:
    c.attr(label='Der Transformationsprozess\n(Systemisch & Ganzheitlich)', bgcolor='#f4f6f7', color='#34495e', fontname='Arial Bold')
    
    c.node('C1', 'Schöpferische Zerstörung\n& Disruption\n(Zukünftige Lösungen ungewiss)', fillcolor='#34495e', fontcolor='white')
    c.node('C2', 'Anpassung von\nAngebots- & Nachfragestrukturen', fillcolor='#ecf0f1')
    c.node('C3', 'Physische Erneuerung\ndes Kapitalstocks\n(Anlagen, Qualifikationen)', fillcolor='#ecf0f1')
    
    c.edge('C1', 'C2')
    c.edge('C2', 'C3')

# --- SÄULEN DER TRANSFORMATION ---
with dot.subgraph(name='cluster_pillars') as c:
    c.attr(label='Die zwei zentralen Säulen', bgcolor='#eaf2f8', color='#2980b9', fontname='Arial Bold')
    
    # Grüne Transformation
    c.node('P1', 'Grüne Transformation\n(Dekarbonisierung &\nerneuerbare Energien)', fillcolor='#27ae60', fontcolor='white')
    
    # Digitale Transformation
    c.node('P2', 'Digitale Transformation\n(Daten als neuer Produktionsfaktor /\ntechnologische Revolution)', fillcolor='#2980b9', fontcolor='white')

# --- FRIKTIONEN / BARRIEREN ---
with dot.subgraph(name='cluster_frictions') as c:
    c.attr(label='Herausforderungen & Friktionen', bgcolor='#fdf2e9', color='#e67e22', fontname='Arial Bold')
    c.node('F1', 'Paradigmatische\nUngewissheit', fillcolor='#fadbd8')
    c.node('F2', 'Koordinationsprobleme &\nhohe Wechselkosten', fillcolor='#fadbd8')
    c.node('F3', 'Strukturelle Rigiditäten &\nStatus-quo-Bias', fillcolor='#fadbd8')

# --- EXTERNE EINFLUSSFAKTOREN ---
with dot.subgraph(name='cluster_externals') as c:
    c.attr(label='Externe Treiber & Rahmenbedingungen', bgcolor='#fef9e7', color='#f1c40f', fontname='Arial Bold')
    c.node('E1', 'Geopolitische Neuordnung\n(Fragmentierung, Souveränität)', fillcolor='#fcf3cf')
    c.node('E2', 'Gesellschaftliche Polarisierung\n(Mangelnder Konsens)', fillcolor='#fcf3cf')
    c.node('E3', 'Zeitliche Dimension\n(Notwendigkeit hoher Anfangsdynamik)', fillcolor='#fcf3cf')
    c.node('E4', 'Kultureller Prozess\n(Fortschrittsnarrativ & Mentalitätswandel)', fillcolor='#fcf3cf')

# --- ZIEL / ZUKUNFT ---
dot.node('END', 'Neuer Entwicklungspfad\n(Zivilisatorischer Prozess / Anthropozän\n& Transhumanismus)', fillcolor='#8e44ad', fontcolor='white')

# --- STRUKTURELLE VERBINDUNGEN (Edges) ---

# Vom alten Pfad in den Prozess
dot.edge('START', 'C1', label=' Impuls durch\n Disruption')

# Einbindung der Säulen in den physischen Umbau
dot.edge('C2', 'P1', style='dashed')
dot.edge('C2', 'P2', style='dashed')
dot.edge('P1', 'C3')
dot.edge('P2', 'C3')

# Einfluss der externen Faktoren auf den gesamten Kernprozess
dot.edge('E1', 'C1', color='#f39c12')
dot.edge('E2', 'C2', color='#f39c12')
dot.edge('E3', 'C1', color='#f39c12')
dot.edge('E4', 'C3', color='#f39c12')

# Friktionen bremsen/wirken auf den Prozess ein
dot.edge('F1', 'C1', color='#c0392b', style='dotted', arrowhead='dot')
dot.edge('F2', 'C2', color='#c0392b', style='dotted', arrowhead='dot')
dot.edge('F3', 'C3', color='#c0392b', style='dotted', arrowhead='dot')

# Erfolgreicher Brückenschlag zum neuen Pfad
dot.edge('C3', 'END', label=' Erfolgreicher\n Pfadwechsel', style='bold')

# --- RENDERN UND ANZEIGEN ---

# 1. Als SVG-Datei lokal speichern
output_name = 'transformationsprozess_modell'
dot.render(output_name, cleanup=True)

# 2. Direkt in IPython anzeigen
#display(SVG(f"{output_name}.svg"))
```

![Transformationsherausforderungen nach @vopel_grundzuge_2025](transformationsprozess_modell.svg)
...
### Ordnungspolitik in der Transformation?
...
# Definition des Digraphs mit passenden Formatierungen
dot = graphviz.Digraph(comment='Eucken vs Voepel', format='svg')

# Globale Attribute für ein sauberes, modernes Layout
dot.attr(rankdir='LR')
dot.attr('node', fontname='Arial', fontsize='11', shape='box', style='filled', color='#2c3e50')
dot.attr('edge', fontname='Arial', fontsize='10', color='#7f8c8d')

# --- CLUSTER 1: Euckens Konstitutive Prinzipien (Wohlgemuth) ---
with dot.subgraph(name='cluster_eucken') as c:
    c.attr(label='Euckens Ordnungspolitik\n(Klassisch / Statisch)', bgcolor='#ebf5fb', color='#2980b9', fontname='Arial Bold')
    c.node('E_main', 'Regelbasierter Rahmen\n& Wettbewerb', fillcolor='#3498db', fontcolor='white')
    c.node('E1', 'Konstanz der Politik\n(Verlässliche Regeln)', fillcolor='#aed6f1')
    c.node('E2', 'Staat als Schiedsrichter\n(Keine Strukturpolitik)', fillcolor='#aed6f1')
    c.node('E3', 'Strikte Haftung\n(Dezentrales Risiko)', fillcolor='#aed6f1')

    c.edge('E_main', 'E1')
    c.edge('E_main', 'E2')
    c.edge('E_main', 'E3')

# --- CLUSTER 2: Gemeinsame Schnittmenge ---
with dot.subgraph(name='cluster_shared') as c:
    c.attr(label='Übereinstimmungen\n(Der ordoliberale Kern)', bgcolor='#e8f8f5', color='#27ae60', fontname='Arial Bold')
    c.node('S1', 'Dezentralität &\nMarktprozesse\n(Skepsis ggü. Planwirtschaft)', fillcolor='#2ecc71', fontcolor='white')
    c.node('S2', 'Offene Märkte &\nfreier Zutritt\n(Gegen Monopole & Lobbyismus)', fillcolor='#2ecc71', fontcolor='white')
    c.node('S3', 'Primat der\nRahmenbedingungen\n(Regeln statt Einzelprozess)', fillcolor='#2ecc71', fontcolor='white')

# --- CLUSTER 3: Vöpels Transformative Ordnungspolitik ---
with dot.subgraph(name='cluster_voepel') as c:
    c.attr(label='Vöpels Ordnungspolitik\n(Transformativ / Dynamisch)', bgcolor='#fef9e7', color='#f39c12', fontname='Arial Bold')
    c.node('V_main', 'Politischer und endogener Transformationsdruck\n(Kreative Zerstörung und politische Anforderungen)', fillcolor='#e67e22', fontcolor='white')
    c.node('V1', 'Flexible Anpassung\n(Agile Regeln im Wandel)', fillcolor='#fadbd8')
    c.node('V2', 'Aktiver Staat\n(Infrastruktur & Skalierung)', fillcolor='#fadbd8')
    c.node('V3', 'Systemische Flankierung\n(Risikoabfederung)', fillcolor='#fadbd8')

    c.edge('V_main', 'V1')
    c.edge('V_main', 'V2')
    c.edge('V_main', 'V3')

# --- Verbindungen & Spannungsverhältnisse (Edges) ---

# Brücken zu den Gemeinsamkeiten
dot.edge('E_main', 'S1', style='dashed', color='#27ae60')
dot.edge('V_main', 'S1', style='dashed', color='#27ae60')
dot.edge('E_main', 'S2', style='dashed', color='#27ae60')
dot.edge('V_main', 'S2', style='dashed', color='#27ae60')
dot.edge('E_main', 'S3', style='dashed', color='#27ae60')
dot.edge('V_main', 'S3', style='dashed', color='#27ae60')

# Spannungsfelder (Konflikte) zwischen den Prinzipien
dot.edge('E1', 'V1', label='Spannung:\nStarrheit vs. Dynamik', color='#c0392b', arrowhead='none', style='bold')
dot.edge('E2', 'V2', label='Spannung:\nSchiedsrichter vs. Gestalter', color='#c0392b', arrowhead='none', style='bold')
dot.edge('E3', 'V3', label='Spannung:\nEigenhaftung vs. Abfederung', color='#c0392b', arrowhead='none', style='bold')

# --- Rendern und Anzeigen ---

# 1. Als SVG-Datei auf der Festplatte speichern
output_filename = 'eucken_vs_voepel_ordnungspolitik'
dot.render(output_filename, cleanup=True)

# 2. Direkt in IPython / Jupyter anzeigen
#display(SVG(f"{output_filename}.svg"))
```

![Ordnungspolitik in der Transformation nach @wohlgemuth_was_2017 und @vopel_grundzuge_2025](eucken_vs_voepel_ordnungspolitik.svg)

## Sozialpolitik

- Sozial-/ verteilungspolitische Flankierung

- Tradeoff mit Effizienz?

- Nach welchen Maßstäben sollen Nutzen und Lasten der Transformation verteilt werden?
